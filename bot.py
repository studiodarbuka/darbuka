...
async def on_ready():
    ...
    three_week_test = now.replace(hour=15, minute=30, second=0, microsecond=0)  # Step1
    two_week_test   = now.replace(hour=15, minute=31, second=0, microsecond=0)  # Step2
    one_week_test   = now.replace(hour=15, minute=32, second=0, microsecond=0)  # Step3
